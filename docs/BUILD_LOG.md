...
## [2026-09-26 13:33 JST] Submission: future work, tech stack and PNG figures

**Goal**: Add a Future section (100–150 words, grounded in the README Limitations and the spec's stretch goals) and a tech-stack list to `docs/SUBMISSION.md`. Export a 1600-pixel-wide PNG of each of the five figures for the form's Images field, generated by the script in `script/plots/`. Pick the cover image.

**Result**:
- `docs/SUBMISSION.md`:
  - §6 Future, 149 words: calibration across scenarios (S2), adaptive τR, fee-sensitive order flow, size-aware fees (S1), and an audit before mainnet with per-chain retuning (S6).
  - §7 Tech stack, grouped the way submission forms usually ask, including what is not used.
  - An Images section with the upload order.
  - New sources-table rows for the added numbers.
- `script/plots/make_figures.py` now renders each figure to PNG right after its SVG. It renders at 300 dpi and downsamples with Lanczos to exactly 1600 px wide. Pillow is already pinned in `requirements.txt`, so no new dependency.
  - Output: `fee-curve.png` 1600×1172, `fee-timeseries.png` 1600×851, `lp-performance.png` 1600×681, `attack-defense.png` 1600×833, `ref-tau-sensitivity.png` 1600×1183. About 1.5 MB in total.
  - Two runs gave byte-identical PNGs and SVGs.
  - The four SVGs whose figures did not change are byte-identical to before.
- Cover: `fee-curve.png`. It shows the whole mechanism in one picture: the two directions split around the reference, volatility lifts both, and the fee stays inside its bounds. It is computed from the contract's formula, not from simulation, so it makes no performance claim.
- README Repository Guide: the Figures row mentions the PNGs.

**Issues**:
1. The PNG export exposed a layout defect in `ref-tau-sensitivity`. The bottom annotation ran over the 900 s line and the 60 s error bar, and the legend ran into the 900 s error bar. The committed SVG, rendered with WebKit, showed the annotation overlap too. Fix, layout only:
   - The annotation is now a subtitle above the plot, with true minus signs.
   - The legend moved below the axes.
   - The default-τR label moved to the left of its line.
   The data and the numbers are unchanged.
2. The first draft of Future said "the defaults were tuned on a single price generator". Only τR was calibrated; the other defaults are placeholders. Corrected.
