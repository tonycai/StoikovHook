...
- **v4-template: the unmodified example hook mines to an address that is already taken on public testnets.** This is expected behavior, not a bug: Dayitva from the Uniswap team confirmed it in a public Discord channel. Thanks to Dayitva for the quick answer. HookMiner is deterministic: for the same deployer (the CREATE2 Deployer Proxy), creation code, constructor arguments and flags, it always returns the same salt and address. Anyone who deploys the template's example hook exactly as generated therefore targets an address that someone else has already deployed. A different creation code (including different compiler settings), different constructor arguments or different flags gives a new address. `HookMiner.find` also skips candidates that already have code (`lib/uniswap-hooks/lib/v4-periphery/src/utils/HookMiner.sol:36`), but that check only sees the chain state the script runs against.
  Our own notes first listed this as an environment problem, with the workaround "change `optimizer_runs` to change the bytecode". No workaround is needed for a real hook: StoikovHook's creation code differs from the example's, so its address was new.
  *Suggestion:* add a note to the template README: the example hook's address is already taken on public testnets, and changing the hook, its constructor arguments or its flags is what produces a fresh address.
